...
import 'package:wisatabumnag/features/event/presentation/blocs/event_order/event_order_bloc.dart';
...
class EventOrderPage extends StatelessWidget with FailureMessageHandler {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<EventOrderBloc>()
        ..add(
      ...
      child: Scaffold(
        ...
        body: BlocListener<EventOrderBloc, EventOrderState>(
              ...
              builder: (context, state) {
                ...
                return SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      DetailPesananWidget(eventDetail: eventDetail),
                      Divider(
                        thickness: 8.h,
                        color: AppColor.grey,
                      ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          BlocBuilder<EventOrderBloc, EventOrderState>(
            builder: (context, state) {
              return InkWell(
                onTap: () async {
                  final bloc = context.read<EventOrderBloc>();
                  final date = await showDatePicker(
                    context: context,
                    initialDate: state.eventDetail?.startDate ?? DateTime.now(),
                    firstDate: state.eventDetail?.startDate ?? DateTime.now(),
                    lastDate: state.eventDetail?.endDate ??
                        DateTime.now().add(
                  ...
                  log(date.toString());
                  if (date != null) {
                    bloc.add(EventOrderEvent.orderForDateChanged(date));
                  }
                },
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        flex: 8,
                        child: BlocSelector<EventOrderBloc, EventOrderState,
                            DateTime>(
                          selector: (state) {
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                onPressed: () {
                                  context.read<EventOrderBloc>().add(
                                        EventOrderEvent.ticketAddButtonPressed(
                                          e,
                                        ),
                                      );
                                },
